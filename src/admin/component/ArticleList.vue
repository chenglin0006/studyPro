    ...
    <div>
        ...
        <div class="router-view">
            <transition name="fade" mode="out-in">
                <router-view @updateLabelNum="updateLabelNum" @saveArticleInformation="refreshArticleList"></router-view>
            </transition>
        </div>
    </div>
</template>
...
export default{
    ...
    methods: {
        ...
        updateLabelNum: function(){
            var self = this;
            self.$http.post('/api/clearTagNum').then(
                respone => {
                    self.articleList.forEach(function(ele,index){
                        self.$http.post('/api/updateTagNum', {
                            label: ele.label
                        }).then(
                            respone => {},
                            respone => {}
    ...
    watch: {
        '$route': function(){
            console.log(this.$route.path.endsWith('articleList'));
            if(this.$route.path.endsWith('articleList')){
                this.fetchData();
            }
        }
    },
